<%@page import="com.gm.gssm.gmfleet.cq.components.content.ComponentCntDealerMapC2GMF"%><%
%><%@include file="/apps/gmfleet/wcm/global.jsp" %><%
%><%
	ComponentCntDealerMapC2GMF comp = new ComponentCntDealerMapC2GMF(pageContext);
	pageContext.setAttribute("comp", comp);
%>
<div class="cnt_dealermap_2 section" data-all-dealer-link="${comp.allDealersLink}">
	<!-- Form -->
	<div class="form_container">
		<div class="infotext"><gmds:out value="${comp.introText}" filter="adam" /></div>
		<div class="form_elements">
			<input class="gmv_zipcode" maxlength="5" value="${comp.searchBoxDefaultText}" onfocus="if (this.value == '${comp.searchBoxDefaultText}') {this.value = '';}" onblur="if (this.value == '') {this.value = '${comp.searchBoxDefaultText}';}" />
			<button class="gmv_infolink arrow btn_prim"><span><gmds:out value="${comp.searchCtaLabel}"/></span></button>
			<c:if test="${comp.businessEliteDealers}">
				<input type="checkbox" value=""><span>${comp.businessEliteDealersLabel}</span>				
			</c:if>
			<gmds:link linkModel="${comp.advancedSearchUrlLink}">
				<gmds:out value="${comp.advancedSearchLabel}"/>
			</gmds:link>
		</div>
		<div class="service_loading"></div>
	</div>
	<!-- Map/Results -->
	<div class="results_container" style="display:none;">
		<!-- Google Map -->
		<div id="gmv_dealermap" style="width:100%;height:475px"><!-- filled by google maps --></div>
		<!-- Results Info Overlay -->
		<div class="info_container">
			<div class="dealer_info">
				<!-- filled by js -->
			</div>			
			<div class="dealer_shopping_links">
				<!-- filled by js -->
			</div>
			<div class="dealer_service_links">
				<!-- filled by js -->
			</div>
		</div>
		<div class="dealer_all_results_container">
			<p class="dealer_results"><!-- filled by js --></p>
			<a href="#" class="btn_sec">${comp.viewAllCTADealersLabel}</a>
		</div>
	</div>	
	<script type="text/javascript" src="//maps.google.com/maps/api/js?sensor=true"></script>
	<script type="text/javascript">
		// dynamic overrides of options from jsp
		mrm.conf.cmp.cnt_dealermap_2 = mrm.$.extend({}, mrm.conf.cmp.cnt_dealermap_2 || {}, {
			showDealerWebsiteLink: ${comp.dealerWebsiteLinkVisible}, // true/false
			dealerWebsiteLabel:'${comp.dealerWebsiteLinkLabel}', // string
			showDrivingDirections:${comp.drivingDirectionsVisible}, // true/false
			drivingDirectionsLabel:'${comp.drivingDirectionsLabel}', //string
			mapHeight:'${comp.mapHeight}',
			mapWidth:'${comp.mapWidth}',
			shoppingLinks: [			
				<c:set var="initpass" value="true"/>
				<c:forEach var="shoppinglink" items="${comp.shoppingToolItems}" varStatus ="status">
				<c:if test="${not empty shoppinglink.url}">
					<c:if test="${initpass ne 'true'}">,</c:if>
					<c:set var="initpass" value="false"/>
						{
							label:'${shoppinglink.labelText}',
							url:'${shoppinglink.url}'
			...
			serviceLinks: [			
				<c:set var="initpass" value="true"/>
				<c:forEach var="servicelink" items="${comp.serviceToolItems}" varStatus ="status">
				<c:if test="${not empty servicelink.labelText}">
					<c:if test="${initpass ne 'true'}">,</c:if>
					<c:set var="initpass" value="false"/>
						{
							label:'${servicelink.labelText}',
							urlQueryParams:'${servicelink.urlQueryParams}'								
						}	
					</c:if>
				</c:forEach>
			]
		...
		$.extend(mrm.cmp, {
			cnt_dealermap_2: {
				...
				_conf: {
					...
					mapWidth: '525px', //string 100px or 100%
					mapHeight: '196px'
				},
				_advancedLink: '',
				...
				initMapContinue: function() {
					...
					if (self._conf.setInfoWidth)
						self.$dealerMapCmp.find('.info_container').width("345px");
				...
				executeAJAX: function() {
					...
					var _returnjson = '',
						...
						_externalDomain = "https://www.gmfleet.com";
					$.ajax({
						...
						success: function(_data, _response) {
							...
							if($( "input:checked" ).val() == false){
								var filter = 'bussinesEliteDealers';
							} else {
								var filter = 'all';
							};
							...
							for (var _count = 0; _count < _data.length; _count++) {
								...
								if ((filter == 'all') || (filter == '')) {
									...
									_trueCount++;
								} else if (filter == 'bussinesEliteDealers') {
				...
				createBinds: function() {
					...
					self.$dealerMapCmp.find('div.form_elements button.arrow').on('click', function(e) {
						self.checkZipCode(e)
					});
